...
const { EmbedBuilder, ActionRowBuilder, SelectMenuBuilder, InteractionCollector} = require("discord.js");
...
module.exports = {
  ...
  options: [
    {
      name: 'command',
      description: 'Get more information about a specified command',
      type: 3,
      required: false,
  ...
  run: async (client, interaction, message, args) => {
    if (interaction.options.get('command').value) {
      ...
      if (!cmd) {
        return interaction.reply({content: "Couldn't find that command!", ephemeral: true })
      } else if (cmd) {
    ...
    } else if (!interaction.options.get('command').value) {
    ...
    .setThumbnail(client.user.displayAvatarURL())
      ...
      interaction.reply({ embeds: [editEmbed], components: [helpMenu], ephemeral: true }).then(msg=>{
