  ...
  fs.readdirSync("./slashCommands/").forEach((dir) => {
    ...
    for (let file of commands) {
      ...
      if (pull.name) {
        ...
        console.log(
          `[HANDLER - SLASH] Loaded a file: ${pull.name} (#${client.slashcmds.size})`
            .brightGreen
        );
        ...
        cmds.push({
          ...
          default_permission: pull.default_permission
            ? pull.default_permission
            : null,
          default_member_permissions: pull.default_member_permissions
            ? PermissionsBitField.resolve(
                pull.default_member_permissions
              ).toString()
            : null,
        });
      } else {
  ...
  (async () => {
    try {
      if (config.Handlers.GUILD_ID) {
        ...
        console.log(
          `[HANDLER - SLASH | INFO] Slash commands has been registered successfully to the guild`
            .green + "\n"
        );
      } else {
